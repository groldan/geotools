...
class ShapefileIndexerBoundsHelper {
     ...
     * bytes per envelope).
     */
    private static final int FALLBACK_TO_FILE_RECCOUNT_THRESHOLD = (1024 * 1024) / 32;
    ...
    static BoundsReader createBoundsReader(final ShapefileReader reader, final IndexFile shpIndex) {
        ...
        final int fileRecCountThreshold =
                (pointBounds ? 2 : 1) * FALLBACK_TO_FILE_RECCOUNT_THRESHOLD;
        ...
        try {
            ...
            LOGGER.log(
                    Level.INFO,
                    "Unable to create a temporary file backed bounds reader, falling back to slower ShapefileReader randome access",
                    e);
     ...
     * #expand(int, Envelope)} provided quick access to each record bounds.
     *
    ...
    static interface BoundsReader {
        ...
        /** Initializes the provided envelope to the bounds of the requested shapefile record. */
        void read(int recNumber, Envelope env) throws IOException;

        /** Expands the provided envelope to cover the bounds of the shapefile record number. */
        void expand(int recNumber, Envelope env) throws IOException;
    ...
    /**
     * A fallback strategy to use {@link ShapefileReader}'s random access to records (e.g. {@link
     * ShapefileReader#recordAt(int)}).
     *
     * <p>{@link #insert(int, Envelope)} is a no-op operation, {@link #read} and {@link #expand}
     * will incur into random disk reads as they position on the {@code .shp} record index and read
     * them to obtain each record bounds.
     */
    private static class ShapefileReaderBoundsReader implements BoundsReader {
        ...
        @Override
        public void expand(int recNumber, Envelope env) throws IOException {
            int offset = shpIndex.getOffsetInBytes(recNumber);
            reader.goTo(offset);
            Record rec = reader.nextRecord();
    ...
    private static class FileBoundsReader implements BoundsReader {
        ...
        @Override
        public void expand(int recNumber, Envelope env) throws IOException {
            reader.expand(recNumber, env);
        }

        private void checkAvailableSpace(final Path path, final int fileSize)
